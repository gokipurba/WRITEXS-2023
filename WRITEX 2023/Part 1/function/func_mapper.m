...
function [out_simbol,code_bit,code_simbol]=func_mapper(bit,mode)
out_simbol=[];
switch mode
     case 'c-bpsk'
        % Mapping bit input
        for i=1:length(bit)
            out_simbol(end+1,1)=(-sqrt(-1)/sqrt(2))*((1-2*bit(i))+sqrt(-1)*(1-2*bit(i)));
        end
        % Mapping buku
        code_bit=[];
        code_simbol=[];
        for isimbol=0:1
            ...
            code_simbol(isimbol+1,1)=(-sqrt(-1)/sqrt(2))*((1-2*isimbol)+sqrt(-1)*(1-2*isimbol));
        end
        
    case 'bpsk'
        % Mapping bit input
        for i=1:length(bit)
            out_simbol(i)=(1/sqrt(2))*((1-2*bit(i))+1i*(1-2*bit(i)));
        end
        % Mapping buku
        code_bit=[];
        code_simbol=[];
